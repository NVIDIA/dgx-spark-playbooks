...
> Fused RMSNorm and cross-entropy that cut training memory and latency
...
Your hardware platform's Blackwell GPU is an ideal environment for profiling and optimizing GPU kernels used during model training. This playbook walks through a real optimization workflow: **profiling a LLaMA 3.1 8B fine-tuning run to identify bottlenecks, then writing custom Triton kernels that eliminate those bottlenecks** — specifically a fused RMSNorm and a fused cross-entropy loss using online softmax.

For inference workloads, tools like `torch.compile` and serving frameworks (vLLM, TensorRT-LLM) already ship highly optimized fused kernels. Training workloads are different. Backward passes double the kernel count, large vocabularies create massive intermediate tensors during loss computation, and `torch.compile` does not restructure algorithms to avoid these allocations. Projects like [Liger-Kernel](https://github.com/linkedin/Liger-Kernel) and [Unsloth](https://github.com/unslothai/unsloth) demonstrate that custom training kernels deliver real results: 20-60% memory reduction and 10-30% throughput improvement.
...
You'll profile a LLaMA 3.1 8B fine-tuning workload, identify the key performance bottlenecks, and write custom Triton kernels that address them on your **hardware platform**.
...
- Familiarity with the [Build Custom CUDA Kernels](https://build.nvidia.com/playbooks/playbook-kernel-dev) playbook (CUDA C++ companion; not required)
...
| :---- | :---- | :---- | :---- | :---- |
| **DGX Station** | DGX OS (Linux) | 252 GB HBM3e (GB300) | Local `kernel-dev-ft` image from playbook Dockerfile (`nvcr.io/nvidia/pytorch:26.01-py3` base); pin GPU with `--gpus '"device=N"'` | — |
...
All required assets are in the playbook directory [in the playbook repository](https://github.com/NVIDIA/dgx-spark-playbooks/blob/main/nvidia/playbook-kernel-dev-ft/).
...
Identify the GB300's device index so the container can target it explicitly. On multi-GPU hardware platforms, pinning to a single, known GPU keeps profiling and benchmark numbers consistent across runs:
...
## On a single-GPU hardware platform you may substitute --gpus all.
docker run -it --rm \
  --name kernel-dev-ft \
  --gpus '"device=N"' \
  --ipc host \
...
> [!IMPORTANT]
> Targeting the GB300 explicitly with `--gpus '"device=N"'` (rather than `--gpus all`) ensures `torch.cuda` and `nvidia-smi` inside the container both see the **GB300** as device `0`. Profiling and benchmark numbers later in this playbook assume a single Blackwell GPU; mixing another workstation GPU in via `--gpus all` can change scheduling and skew tokens/sec and bandwidth utilization figures.
...
- **Total: ~96 GB minimum**, fitting comfortably in available HBM on supported hardware platforms
...
> [!NOTE]
> **Ranking variance:** The exact ordering and percentages in the "Top 20 CUDA operations" table can change between runs, PyTorch / CUDA builds, and GPU generation. You should still see the same *categories* of work (matmuls, FlashAttention, RMSNorm decompositions, cross-entropy). **"Command Buffer Full"** (or similar) sometimes appears at the top of self-time tables: that reflects the GPU driver's **submission queue / scheduling**, not a user kernel to optimize. The script filters that row from the printed table; the raw trace in Perfetto still contains the underlying kernels.
...
GB300 HBM3e peak bandwidth is ~8 TB/s. PyTorch's unfused RMSNorm typically achieves only ~11% of this peak. A fused kernel can reach ~80-90% — a dramatic improvement for an operation that runs 66 times per training step.
...
> [!NOTE]
> Same caveat as in Step 7: **absolute microseconds in the example table are illustrative**. On validation runs for this playbook the per-iteration latencies were several thousand µs rather than the ~300 µs printed above, while the **memory reduction** (~6x) and the **speedup direction** (fused becomes faster as `num_tokens` grows) remained stable. Trust the **shape** of the table and the **memory column**, not the absolute latency numbers.
...
> [!NOTE]
> **Treat the throughput numbers above as illustrative, not a target** — same caveat as the RMSNorm (Step 7) and cross-entropy (Step 10) benchmark notes. Absolute tok/s and step time **vary** with GPU generation, clocks, PyTorch / CUDA builds, and whether the warm-up pass included JIT. The **stable signals** are (1) the **relative gap** — optimized > baseline — and (2) the **peak GPU memory delta** (the cross-entropy memory reduction is what frees room for larger batch sizes). Match those, not the absolute tok/s.
...
- **Fused SwiGLU with backward:** The [Build Custom CUDA Kernels](https://build.nvidia.com/playbooks/playbook-kernel-dev) playbook covered inference-only SwiGLU. Training needs the backward pass; use the same `torch.autograd.Function` pattern as here.
- **cuTile kernels:** Explore [Run cuTile Kernels](https://build.nvidia.com/playbooks/playbook-cutile-kernels) for TileGym benchmarks and FMHA implementation workflows.
...
| `triton.compiler.errors.CompilationError` referencing `sm_100` | Triton build lacks Blackwell support | Use the playbook Dockerfile (PyTorch NGC `26.01-py3` base), which includes Triton with Blackwell support. Verify: `python -c "import triton; print(triton.__version__)"`. |
...
| `RuntimeError: Trace is already saved` from profiler | Profiler exported the Chrome trace more than once in the same process | Use the playbook's `profile_baseline.py`, which exports the Chrome trace exactly once and clears any prior Chrome trace for the same flags before recording. |

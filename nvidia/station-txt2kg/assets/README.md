...
* **LLM Inference**
  * **Ollama**: Local LLM inference with GPU acceleration
    * Default model: `llama3.1:405b` (optimized for GB300 Ultra)
...
* **vLLM Stack** (with `--vllm` flag)
  * **vLLM**: GPU-accelerated LLM inference optimized for DGX Station GB300
...
- **High-performance inference**: Optimized for DGX Station GB300 Ultra
...
- Model: `llama3.1:405b` (leverages GB300's large GPU memory)
...
3. **Pull the Llama 3.1 405B model (first time only):**
```bash
...
### Alternative: Using vLLM (for DGX Station GB300)

...
# Nodes whose seed widget is followed by a frontend-only "control_after_generate"
# widget, mapped to the name of that seed input.
#
# The UI serializes widgets_values as a positional list with no names, and the
# frontend inserts control_after_generate immediately after the seed. API graphs
# have no such key — it tells the UI what to do with the seed after a run, which
# is meaningless to the backend — so it must be synthesized here. Omitting it
# shifts every later value into the wrong widget: cfg receives sampler_name's
# string, sampler_name receives scheduler's, and the node fails to load.
SEED_CONTROL_WIDGETS: dict[str, str] = {
    "KSampler": "seed",
    "KSamplerAdvanced": "noise_seed",
    "RandomNoise": "noise_seed",
}

# "fixed" keeps the seed baked into the API graph, so a converted workflow
# reproduces the same output the API graph produces. Use "randomize" for a
# fresh result on every Queue.
SEED_CONTROL_DEFAULT = "fixed"
...
def build_ui_workflow(api: dict[str, Any]) -> dict[str, Any]:
    ...
    for order, nid in enumerate(node_ids):
        ...
        seed_field = SEED_CONTROL_WIDGETS.get(ctype)
        seed_widget_idx: int | None = None
        ...
        for slot_idx, (iname, val) in enumerate(input_items):
            ...
            if is_link(val):
                ...
                if iname == seed_field:
                    seed_widget_idx = len(widgets_values)
                widgets_values.append(val)

        # See SEED_CONTROL_WIDGETS: the frontend expects this widget directly
        # after the seed, and the API graph cannot supply it.
        if seed_widget_idx is not None:
            widgets_values.insert(seed_widget_idx + 1, SEED_CONTROL_DEFAULT)

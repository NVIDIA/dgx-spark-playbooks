...
> Ready-to-run workflows you can copy and personalize
...
This playbook is a companion to [Run NemoClaw with a Local LLM](https://build.nvidia.com/playbooks/nemoclaw). It walks through **four ready-to-run applications** you can stand up on top of an existing NemoClaw sandbox — a personal morning news digest, a software development agent, a doc and deck red-team, and a calendar negotiation chief-of-staff.
...
- You have completed [Run NemoClaw with a Local LLM](https://build.nvidia.com/playbooks/nemoclaw) and have a working sandbox (the examples use `my-assistant`)
- Basic comfort with the Linux terminal and YAML files
- Awareness of the security and risk guidance in the NemoClaw playbook overview

**Optional:**

- A Telegram bot for messaging-driven workflows (required for News Digest delivery over Telegram; optional for Calendar Negotiator proxy modes and for review notifications from Software Development Agent / Deck Reviewer)
...
| :---- | :---- | :---- | :---- | :---- |
| **DGX Spark** | DGX OS (Linux) | 128 GB Unified Memory | Working NemoClaw sandbox with local inference (see companion playbook) | — |
...
> [!IMPORTANT]
> If you skipped Telegram at the NemoClaw onboard step, `nemoclaw $SANDBOX_NAME policy-add` will open the egress preset but the bot will still reply `Error: Channel is unavailable: telegram`. The channel **plugin** is not added by `policy-add`. Register it with `nemoclaw $SANDBOX_NAME channels add telegram`, or follow the messaging-channel steps in [Run NemoClaw with a Local LLM](https://build.nvidia.com/playbooks/nemoclaw).

Open Telegram, find your bot, and send `hello`. You should get a reply from the local model within 30–90 seconds (first-response cold start can be slow on large models). After that, hand off to the application tab you want to set up.

## Step 2. (Optional) cloudflared for remote Web UI access
...
> [!NOTE]
> For installer-level NemoClaw issues (Docker, local inference, gateway, Telegram setup), see the **Troubleshooting** tab of [Run NemoClaw with a Local LLM](https://build.nvidia.com/playbooks/nemoclaw) before debugging here — most reported issues come from the install layer rather than the application layer.

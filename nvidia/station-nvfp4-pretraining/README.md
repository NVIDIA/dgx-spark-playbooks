...
* **Estimated duration**: 20-30 minutes (quick test loop with default `--train-iters 10`); longer for real data
* **Risks**:
...
| `CUDA out of memory` during model init | Insufficient GPU memory for Llama 1B + optimizer states | Reduce `micro_batch_size` or use `--nproc_per_node` for model parallelism |

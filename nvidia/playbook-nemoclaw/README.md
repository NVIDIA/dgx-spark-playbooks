...
- [Instructions](#instructions)
  ...
  - [Step 5. Set Up NemoClaw Agents](#step-5-set-up-nemoclaw-agents)
  - [Step 6. Update NemoClaw](#step-6-update-nemoclaw)
  - [Step 7. Stop services](#step-7-stop-services)
  - [Step 8. Uninstall NemoClaw](#step-8-uninstall-nemoclaw)
- [Multi-node](#multi-node)
  - [Step 1. Prerequisites](#step-1-prerequisites)
  - [Step 2. Verify the fabric](#step-2-verify-the-fabric)
  - [Step 3. Download and copy the model cache](#step-3-download-and-copy-the-model-cache)
  - [Step 4. Start the head container on node-1](#step-4-start-the-head-container-on-node-1)
  - [Step 5. Start the worker container on node-2](#step-5-start-the-worker-container-on-node-2)
...
- [Troubleshooting](#troubleshooting)
  - [Optional — Messaging (Telegram)](#optional-messaging-telegram)
...
NemoClaw uses **local vLLM** for inference. Express Install selects a maintained model for your hardware platform. Browse tested vLLM launch settings on [vLLM Recipes](https://recipes.vllm.ai/browse).
...
Canonical source: https://github.com/NVIDIA/NemoClaw/blob/522ad1ea9c479eb874e7a7eb33079de2be3919bd/docs/resources/starter-prompt.md
Pinned upstream commit: 522ad1ea9c479eb874e7a7eb33079de2be3919bd
...
Prompt source: [NVIDIA NemoClaw starter prompt](https://github.com/NVIDIA/NemoClaw/blob/522ad1ea9c479eb874e7a7eb33079de2be3919bd/docs/resources/starter-prompt.md)
...
2. **Configuring inference** -- Choose a local inference option to run models on your hardware platform.
3. **Inference models** -- Select a model from the installer prompts. NemoClaw will prepare any required local model artifacts when needed. On DGX Station, Nemotron 3 Ultra is the Express recommendation; DeepSeek-V4-Flash is also supported.
...
7. **Messaging channels** -- Optional. Prefer **No** for a first local install (Web UI / terminal only). Full Telegram setup is at the bottom of this page under **Optional — Set up Messaging Channel (Telegram)**. If you enable a channel now, choose your desired bot (`telegram`, `discord` or `slack`) and paste your bot token when prompted.
...
## Optional — Set up Messaging Channel (Telegram)
...
## Phase 1: Prepare both nodes

### Step 1. Prerequisites
- Complete the connectivity playbook for your DGX Station pair before starting this tab. Both high-speed fabric rails must be configured and validated.
- Hugging Face [Access Token](https://huggingface.co/docs/hub/en/security-tokens) for downloading the model.
- Docker and [NVIDIA Container Toolkit](https://docs.nvidia.com/datacenter/cloud-native/container-toolkit/latest/install-guide.html) installed on both nodes.
- Both nodes must be connected to each other via the validated fabric.
...
## Phase 2: Launch the multi-node inference server

### Step 4. Start the head container on node-1
...
### Step 5. Start the worker container on node-2

Connect to `node-2` and set the worker variables:

>[!NOTE]
> HEAD_IP value should be fetched from node-1 [Step 4](#step-4-start-the-head-container-on-node-1)
...
## Phase 3: Verify the multi-node inference server
...
| CoreDNS crash loop | Known issue on some hardware platform configurations | Re-run the NemoClaw installer (`curl -fsSL https://www.nvidia.com/nemoclaw.sh \| bash`) which includes the CoreDNS fix. If the issue persists, see [NemoClaw troubleshooting](https://docs.nvidia.com/nemoclaw/latest/reference/troubleshooting.html). |
| "No GPU detected" during onboard | Some hardware platforms report unified memory differently | Expected on some hardware platforms. The wizard still works and uses vLLM for inference. |
| Inference timeout or hangs | vLLM not running, the model is still loading, or the server is not reachable | Check the vLLM server: `curl http://127.0.0.1:8000/v1/models` should list the model you selected during onboarding or Express Install. If the request hangs, wait for `Application startup complete`, then check `nemoclaw my-assistant status` for the Inference health line. |

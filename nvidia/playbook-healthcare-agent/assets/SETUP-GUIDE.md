...
```bash
# Uses the package-managed systemd service, native Docker configuration,
# generated mTLS certificates, and HTTPS registration.
bash scripts/ensure_openshell_gateway.sh

# Remote/headless Linux only: persist the user service after SSH logout.
...
The helper owns the `systemctl --user` enable/restart lifecycle and registers
`https://127.0.0.1:17670` as the local mTLS endpoint; do not launch a separate
plaintext gateway process.

Success requires `Status: Connected`, `Authentication: Authenticated`, and a
healthy Docker compute driver. If the helper reports stale systemd Docker-group
membership, fully end all login sessions and log back in or reboot, then rerun.
When linger is already enabled, use a maintenance reboot because logout does
not recreate the user manager. Inspect service failures with

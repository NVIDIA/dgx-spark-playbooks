...
* **Neo4j + Ollama Stack** (with `--neo4j` flag)
  * **Neo4j**: Alternative graph database
  * **Ollama**: Local LLM inference using the same model flow as the default stack
...
DGX Spark is not affected by the 64 KB page-size issue. On affected DGX Station systems, use `./start.sh --neo4j` instead to avoid ArangoDB container images that may abort with `<jemalloc>: Unsupported system page size`.
...
### Alternative: Using Neo4j with Ollama

For local Ollama inference with Neo4j as the graph database:
```bash
./start.sh --neo4j
```
...
- **Switch graph database**: Use `--neo4j` for Neo4j + Ollama
- **Switch LLM backend**: Use `--vllm` for Neo4j + vLLM or default for Ollama

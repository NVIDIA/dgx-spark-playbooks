...
export class Neo4jService {
  ...
  public async getGraphData(): Promise<{ 
    ...
    const session = this.getSession();
    try {
      // Get knowledge graph nodes only. QueryLog and QueryExecution nodes are
      // internal metrics and should not appear in the user graph.
      ...
      // Get knowledge graph relationships only.
      const relsResult = await session.run(
        'MATCH (source:Entity)-[r:RELATIONSHIP]->(target:Entity) RETURN r, source, target'
      );
  ...
  /**
   * Traverse the knowledge graph from nodes matching the provided keywords.
   */
  public async graphTraversal(
    ...
    const boundedDepth = Math.max(1, Math.min(Math.floor(maxDepth), 5));
    const boundedResults = Math.max(1, Math.floor(maxResults));
    const session = this.getSession();
    ...
    try {
      const result = await session.run(
        `
        MATCH (start:Entity)
        WHERE any(keyword IN $keywords WHERE toLower(start.name) CONTAINS keyword)
        MATCH path = (start)-[:RELATIONSHIP*1..${boundedDepth}]-(neighbor:Entity)
        WITH relationships(path) AS rels, length(path) AS depth
        UNWIND rels AS rel
        WITH rel, startNode(rel) AS source, endNode(rel) AS target, min(depth) AS depth
        RETURN
        ...
        `,
        {
          ...
          limit: neo4j.int(boundedResults)
        }
      );

      return result.records.map(record => {
        const depth = this.toNumber(record.get('depth')) || 1;
        return {
      ...
      console.error('Error traversing graph in Neo4j:', error);
      throw error;
    } finally {
      session.close();
    }
  ...
  public async getQueryLogs(limit: number = 100): Promise<any[]> {
    ...
    try {
      ...
      const result = await session.run(cypher, { limit: neo4j.int(Math.max(1, Math.floor(limit))) });
      ...
      if (result.records.length === 0) {
        ...
        const nodeCount = this.toNumber(checkNodesResult.records[0].get('count'));
        ...
        if (nodeCount > 0) {
          ...
          const relCount = this.toNumber(checkRelsResult.records[0].get('count'));
      ...
      return result.records.map(record => {
        const mappedRecord = {
          ...
          executionCount: this.toNumber(record.get('executionCount'))
        };

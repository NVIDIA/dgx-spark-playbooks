...
export class BackendService {
  ...
  private isVectorSearchEnabled(): boolean {
    return ['1', 'true', 'yes'].includes((process.env.VECTOR_SEARCH_ENABLED || '').toLowerCase());
  }
  ...
  public async initialize(graphDbType?: GraphDBType): Promise<void> {
    ...
    if (!this.isVectorSearchEnabled()) {
      console.log('Vector search is disabled; skipping Qdrant and SentenceTransformer initialization');
      this.initialized = true;
      return;
    }

    // Initialize Qdrant and SentenceTransformer only for vector-enabled workflows.
  ...
  private async generateEmbeddings(texts: string[]): Promise<number[][]> {
    if (!this.isVectorSearchEnabled()) {
      throw new Error('Vector search is disabled. Restart with ./start.sh --vector-search to use embeddings.');
    }
  ...
  public async processTriples(triples: Triple[]): Promise<void> {
    ...
    if (!this.isVectorSearchEnabled()) {
      console.log(`Vector search disabled; stored ${uniqueTriples.length} triples without entity embeddings`);
      return;
    }
  ...
  public async query(
    ...
    if (!this.isVectorSearchEnabled()) {
      throw new Error('Vector search is disabled. Use Graph Search or restart with ./start.sh --vector-search.');
    }
...
Answer:`;
      ...
      // Determine LLM endpoint and model based on provider. If the vLLM stack
      // is running and the UI did not pass a provider, prefer vLLM over the
      // disabled Ollama endpoint.
      const finalProvider = llmProvider || (
        process.env.VLLM_MODEL && process.env.VLLM_MODEL !== 'disabled' && process.env.OLLAMA_MODEL === 'disabled'
          ? 'vllm'
          : 'ollama'
      );
      const configuredModel = finalProvider === 'vllm' ? process.env.VLLM_MODEL : process.env.OLLAMA_MODEL;
      const finalModel = llmModel
        || (configuredModel && configuredModel !== 'disabled' ? configuredModel : undefined)
        || (finalProvider === 'vllm' ? 'nvidia/Llama-3_3-Nemotron-Super-49B-v1_5-FP8' : 'llama3.1:8b');
  ...
  public async storeDocumentChunks(
    ...
    if (!this.isVectorSearchEnabled()) {
      throw new Error('Vector search is disabled. Restart with ./start.sh --vector-search to store document embeddings.');
    }

...
> Reasoning, tool use, and long-context chat behind an OpenAI-compatible API
...
The family lives in the open [Nemotron](https://github.com/NVIDIA-NeMo/Nemotron) project and on [Hugging Face](https://huggingface.co/nvidia) alongside documentation and cookbooks from NVIDIA and the community. This playbook is your on-ramp for **running Nemotron on a single hardware platform**: you get a working inference endpoint, not a tour of every training or export option.

## What you'll accomplish

You will deploy a **production-style Nemotron inference server** on your supported hardware platform with an **OpenAI-compatible HTTP API**, so you can send chat completions from `curl`, scripts, or client libraries. You will use checkpoints and runtimes validated for that hardware’s GPU and memory footprint, with steps for verifying the server and (where the model supports it) reasoning-style behavior.

The instruction tabs are **two supported ways to get there**—different models and engines. You do not need both; choose the tab that matches the model you want.
...
| :---- | :---- | :---- | :---- | :---- |
| **DGX Spark** | DGX OS (Linux) | 128 GB Unified Memory | Nano: `vllm/vllm-openai:v0.20.0` · Super: `vllm/vllm-openai:cu130-nightly` or `nvcr.io/nvidia/tensorrt-llm/release:1.3.0rc9` | — |
...
- Current NVIDIA GPU driver stack for your hardware platform
- Hugging Face account with an access token when the model card requires it

**Choosing a tab**

- **Run Nemotron Nano** — Serve **Nemotron-3-Nano** on Docker with **[vLLM](https://github.com/vllm-project/vllm)** (`vllm/vllm-openai:v0.20.0`), pointing the container at local **Nemotron-3-Nano Omni** weights and running `vllm serve` on port **8000**.
- **Run Nemotron Super** — Serve the **NVFP4** Nemotron-3-Super checkpoint on Docker with **either** **[vLLM](https://github.com/vllm-project/vllm)** (`vllm/vllm-openai:cu130-nightly`, reasoning-parser plugin, tuned env vars and flags) **or** **[TensorRT-LLM](https://github.com/NVIDIA/TensorRT-LLM)** (`nvcr.io/nvidia/tensorrt-llm/release:1.3.0rc9`, `hf download` into a local folder, `extra-llm-api-config.yml`, and `trtllm-serve`).

## Find model recipes

Browse Nemotron checkpoints, cookbooks, and tested launch settings from these catalogs:

| Resource | What you get |
| -------- | ------------ |
| [NVIDIA NeMo Nemotron](https://github.com/NVIDIA-NeMo/Nemotron) | Open project, cookbooks, and deployment guides |
| [Nemotron-3-Nano Omni on Hugging Face](https://huggingface.co/nvidia/Nemotron-3-Nano-Omni-30B-A3B-Reasoning-BF16) | Nano Omni weights and model-card guidance |
| [vLLM Recipe — Nemotron-3-Nano Omni](https://recipes.vllm.ai/nvidia/Nemotron-3-Nano-Omni-30B-A3B-Reasoning-BF16) | Copyable `vllm serve` settings for Nano Omni |
| [Nemotron-3-Super NVFP4 on Hugging Face](https://huggingface.co/nvidia/NVIDIA-Nemotron-3-Super-120B-A12B-NVFP4) | Super NVFP4 checkpoint and card |
| [Nemotron Spark deployment guide (Super)](https://github.com/NVIDIA-NeMo/Nemotron/tree/main/usage-cookbook/Nemotron-3-Super/SparkDeploymentGuide) | Tuned vLLM / TensorRT-LLM recipes for Super |
| [Nemotron on NVIDIA Developer](https://developer.nvidia.com/topics/ai/nemotron) | Family overview and related resources |

Use the **Run Nemotron Nano** or **Run Nemotron Super** tab for the validated single-node workflow on your supported hardware platform.

> [!NOTE]
> **Memory determines what you can run.** Large Super checkpoints need substantially more disk and memory headroom. If a variant is not covered in the instruction tabs, check the model card and cookbooks above before downloading.

## Time & risk

- **Estimated time:** 45 MIN (first-time setup can take longer; downloads of weights and images dominate)
- **Risk level:** Low
  - Work is in user space (Docker, weight directories, cache dirs); you are not asked to reimage the system
  - Model download may require Hugging Face authentication for gated assets
...
- **Last Updated:** 07/31/2026
  - Single-node Nano (vLLM) and Super (vLLM or TensorRT-LLM) serving with OpenAI-compatible API on supported hardware

## Run Nemotron Nano

## Step 1. Overview and prerequisites

This tab serves **NVIDIA Nemotron-3-Nano** on a single hardware platform with **vLLM**, using the upstream multi-arch image **`vllm/vllm-openai:v0.20.0`**. The server exposes an OpenAI-compatible HTTP API on port **8000**. The example uses the multimodal **Nemotron-3-Nano Omni** weights (text, image, audio, and video), so the launch command installs the optional audio packages and enables the multimodal limits.

The batch, context, and cache choices below assume a **~128 GB unified memory** footprint on the supported hardware platform.

This tab covers the **hardware platform setup**. For everything not covered here—**API examples, reasoning mode, and video tuning**—follow the general Nemotron instructions and the model card (see **Find model recipes** in Overview).

**Requirements**

- Supported hardware platform with sufficient disk for weights, caches, and the container image
...
Pull the upstream multi-arch vLLM **v0.20.0** image. Docker selects the architecture that matches your hardware platform.
...
| `--gpu-memory-utilization 0.8` | Fraction of GPU-visible memory vLLM may use |
...
**Key hardware platform flags**

| Flag | Purpose | Guidance |
| ---- | ------- | -------- |
| `--gpus all` | Select GPU | Use all visible GPUs; on a single-GPU platform this is equivalent to `device=0` |
...
On unified-memory hardware platforms, CPU and GPU share one memory pool. If you hit OOM, use these two levers, in order of impact:
...
For the larger **Nemotron Super** deployment on the same hardware platform, use the **Run Nemotron Super** tab in this playbook.

## Run Nemotron Super

## Step 1. Overview and prerequisites

This tab serves **NVIDIA Nemotron-3-Super** (NVFP4 checkpoint) on a single hardware platform. Deploy with **vLLM** (`vllm/vllm-openai:cu130-nightly`) or **TensorRT-LLM** (`nvcr.io/nvidia/tensorrt-llm/release:1.3.0rc9`) using the commands and tuning from the [Nemotron Spark deployment guide](https://github.com/NVIDIA-NeMo/Nemotron/tree/main/usage-cookbook/Nemotron-3-Super/SparkDeploymentGuide).

The guide’s KV, batch, and cache choices assume a **~128 GB unified memory** footprint.
...
- Supported hardware platform with sufficient disk for weights, caches, and images
- NVIDIA Container Toolkit and Docker with GPU access
- Hugging Face token (`HF_TOKEN`) when the model card requires it

**How the steps are grouped**

- **Step 2:** Download the reasoning parser file—required before starting **either** server.
...
MTP + NVFP4 on this hardware platform requires this **cu130 nightly** image.
...
On the host, export the vLLM variables (they match the upstream deployment guide) and your Hugging Face token before `docker run`. Copy and paste:
...
- `VLLM_NVFP4_GEMM_BACKEND` — Marlin GEMM for NVFP4 on this hardware platform.
...
- `VLLM_USE_FLASHINFER_MOE_FP4` — Disable FlashInfer FP4 MoE (multi-GPU Blackwell); Marlin handles FP4 on this single-GPU recipe.
...
| Flag | Role |
| ---- | ---- |
| `--tensor-parallel-size 1` | Single GPU; no expert parallelism in this recipe |
...
For **Nemotron Nano**, use the **Run Nemotron Nano** tab.
...
| FP4 / MoE kernel errors | Wrong image or backend | Use `vllm/vllm-openai:cu130-nightly`. Keep `VLLM_NVFP4_GEMM_BACKEND=marlin`, `VLLM_USE_FLASHINFER_MOE_FP4=0`, and `--moe-backend marlin` / `--quantization fp4` |
| Allreduce / distributed warnings on one GPU | Single-GPU topology | Set `VLLM_FLASHINFER_ALLREDUCE_BACKEND=trtllm` (Steps 4–5) as in the deployment guide; see [vLLM PR #35793](https://github.com/vllm-project/vllm/pull/35793) |
...
| MoE / NVFP4 backend errors | Backend mismatch for single GPU | Keep `moe_config.backend: CUTLASS` in `extra-llm-api-config.yml` for this single-GPU recipe |
...
> [!NOTE]
> Some hardware platforms use a Unified Memory Architecture (UMA), which enables dynamic sharing between GPU and CPU. Some workloads can still hit memory pressure while reporting headroom. If you see unexplained OOM or stalls, try flushing the page cache (administrative host only):

```bash
sudo sh -c 'sync; echo 3 > /proc/sys/vm/drop_caches'
```

For platform known issues on DGX Spark, see the [DGX Spark known issues](https://docs.nvidia.com/dgx/dgx-spark/known-issues.html) page.

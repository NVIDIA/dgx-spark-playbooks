#!/bin/bash
# Build NCCL and the NCCL test suite on every node.
#
# Usage: ./setup.sh [WORKER_IP ...]
#   WORKER_IP ...  (optional) IP/hostname of each additional node (Node 2, 3, 4).
#                  The same build is run on each over SSH. Omit to build locally
#                  only. Works for any node count (2, 3, or 4).
#
...
sudo -S -p '' sh -c 'apt-get update && apt-get install -y libopenmpi-dev' && \
git clone -b ${NCCL_VERSION} https://github.com/NVIDIA/nccl.git \$HOME/nccl/ && \
cd \$HOME/nccl/ && \
...
echo "  2 nodes:  bash launch.sh --topology direct <NODE_1_IP> <NODE_2_IP>"
echo "  3 nodes:  bash launch.sh --topology ring   <NODE_1_IP> <NODE_2_IP> <NODE_3_IP>"
echo "  4 nodes:  bash launch.sh --topology switch <NODE_1_IP> <NODE_2_IP> <NODE_3_IP> <NODE_4_IP>"

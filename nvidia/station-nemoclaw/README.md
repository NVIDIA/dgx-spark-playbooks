...
**NVIDIA NemoClaw** is an open-source reference stack that simplifies running OpenClaw always-on assistants more safely. It installs the **NVIDIA OpenShell** runtime — an environment designed for executing agents with additional security — and connects them to **local vLLM** inference on your DGX Station. A single installer command (`nemoclaw.sh`) detects the DGX Station and offers an **Express Install** that selects the recommended model and policy, installs Node.js, OpenShell, and the NemoClaw CLI, and creates the sandbox.

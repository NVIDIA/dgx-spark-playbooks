...
```text
 ──────────────────────────────────────────────────
  ...
  Model:    deepseek-ai/DeepSeek-V4-Flash (Local vLLM)
...
| Inference timeout or hangs | vLLM not running or not reachable | Check the vLLM server: `curl http://127.0.0.1:8000/v1/models` should list `Qwen/Qwen3.6-27B-FP8`. If it hangs, the model may still be loading — wait for `Application startup complete`. Then check `nemoclaw my-assistant status` for the Inference health line. |

...
[[ "${server_ready_timeout}" =~ ^[1-9][0-9]*$ ]] || \
  die "CX8_PERFTEST_SERVER_READY_TIMEOUT must be a positive integer number of seconds"
CX8_GPU_BDF="${gpu_bdf}"
...
check_gdr_prerequisites_one() {
  local label="$1"
  local host="$2"
  local preflight_log="${log_dir}/${label}_gdr_preflight.log"
  local remote_cmd
  local rc
  remote_cmd="preflight_rc=0; cd '${CX8_REMOTE_BASE}' || exit 1; sudo -p '[sudo on ${label}] password for %u: ' env ${env_assign} ./assets/install_perftest_gdr.sh --check-only || preflight_rc=\$?; sudo -p '[sudo on ${label}] password for %u: ' env ${env_assign} ./assets/configure_acs.sh || preflight_rc=\$?; exit \${preflight_rc}"

  echo
  echo "### ${label}: ${host}"
  set +e
  ssh_mux_tty "${host}" "${remote_cmd}" 2>&1 | tee "${preflight_log}"
  rc="${PIPESTATUS[0]}"
  set -e
  return "${rc}"
}
...
if [[ "${gdr}" == "1" ]]; then
  echo "Checking GDR prerequisites on both DUTs before starting ib_write_bw"
  gdr_preflight_rc=0
  check_gdr_prerequisites_one "${CX8_A_NAME}" "${remote_a}" || gdr_preflight_rc=$?
  check_gdr_prerequisites_one "${CX8_B_NAME}" "${remote_b}" || gdr_preflight_rc=$?

  if [[ "${gdr_preflight_rc}" != "0" ]]; then
    echo
    echo "STOP: GDR prerequisites did not pass on both DUTs. No ib_write_bw GDR server or client was started."
    echo "Complete the documented perftest-capability and ACS/Data Direct preparation, then rerun this command."
    echo "Logs: ${log_dir}"
    exit "${gdr_preflight_rc}"
  fi
  echo "PASS: GDR prerequisites passed on both DUTs"
fi

echo "Starting server on ${CX8_A_NAME} (${remote_a}), rail ${rail}"
ssh_mux_plain "${remote_a}" "mkdir -p '${CX8_REMOTE_BASE}/logs'; pkill -f '[i]b_write_bw.*-d ${server_hca}' >/dev/null 2>&1 || true; cd '${CX8_REMOTE_BASE}'; nohup env ROLE='${CX8_A_ROLE}' ${env_assign} ./assets/run_perftest.sh --server --rail '${rail}' ${gdr_arg} --duration '${duration}' --size '${size}' >'${remote_server_log}' 2>&1 & server_pid=\$!; echo \${server_pid} >'${remote_server_pid}'; cleanup_server() { kill \"\${server_pid}\" >/dev/null 2>&1 || true; wait \"\${server_pid}\" 2>/dev/null || true; rm -f '${remote_server_pid}'; }; trap cleanup_server EXIT; start=\$(date +%s); timeout='${server_ready_timeout}'; while :; do if grep -Eq 'Waiting for client to connect|local address:' '${remote_server_log}' 2>/dev/null; then trap - EXIT; exit 0; fi; if ! kill -0 \${server_pid} >/dev/null 2>&1; then echo 'ERROR: perftest server exited before accepting a client'; cat '${remote_server_log}' 2>/dev/null || true; exit 1; fi; now=\$(date +%s); if [ \$((now - start)) -ge \${timeout} ]; then echo \"ERROR: perftest server was not ready within \${timeout}s\"; cat '${remote_server_log}' 2>/dev/null || true; exit 1; fi; sleep 1; done"
...
ssh_mux_plain "${remote_a}" "if test -s '${remote_server_pid}'; then pid=\$(cat '${remote_server_pid}'); kill \"\${pid}\" >/dev/null 2>&1 || true; fi; rm -f '${remote_server_pid}'" >/dev/null 2>&1 || true

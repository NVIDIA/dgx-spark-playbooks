...
def wrapper_launcher(os_name: str | None = None) -> str:
    """Return the documented launcher for the current control-host platform."""
    platform_name = os.name if os_name is None else os_name
    if platform_name == "nt":
        return r"py -3 .\cx8_setup.py"
    return "python3 ./cx8_setup.py"
...
def load_config() -> Config:
    ...
    if re.fullmatch(r"[1-9][0-9]*", cfg.perftest_server_ready_timeout) is None:
        raise SystemExit(
            "ERROR: CX8_PERFTEST_SERVER_READY_TIMEOUT must be a positive "
...
def cmd_perftest(cfg: Config, args: argparse.Namespace) -> int:
    ...
    if args.gdr:
        print("Checking GDR prerequisites on both DUTs before starting ib_write_bw")
        preflight_rc = 0
        for label, host, _role in for_each_station(cfg):
            check_perftest = remote_asset(
                cfg, "install_perftest_gdr.sh", sudo=True, extra="--check-only"
            )
            check_acs = remote_asset(cfg, "configure_acs.sh", sudo=True)
            preflight_command = (
                f"preflight_rc=0; {check_perftest} || preflight_rc=$?; "
                f"{check_acs} || preflight_rc=$?; exit $preflight_rc"
            )
            station_rc = run_logged(
                cfg,
                host,
                preflight_command,
                d / f"{label}_gdr_preflight.log",
                tty=True,
            )
            if station_rc != 0:
                preflight_rc = station_rc
        if preflight_rc != 0:
            print(
                "\nSTOP: GDR prerequisites did not pass on both DUTs. "
                "No ib_write_bw GDR server or client was started.\n"
                "Complete the documented perftest-capability and ACS/Data Direct "
                "preparation, then rerun this command.",
                file=sys.stderr,
            )
            print(f"\nLogs: {d}")
            return preflight_rc
        print("PASS: GDR prerequisites passed on both DUTs")
    server_cmd = (
        ...
        f"cleanup_server() {{ kill \"$server_pid\" >/dev/null 2>&1 || true; "
        f"wait \"$server_pid\" 2>/dev/null || true; rm -f {q(remote_server_pid)}; }}; "
        f"trap cleanup_server EXIT; "
        f"start=$(date +%s); timeout={q(cfg.perftest_server_ready_timeout)}; "
        f"while :; do "
        f"if grep -Eq 'Waiting for client to connect|local address:' {q(remote_server_log)} 2>/dev/null; then trap - EXIT; exit 0; fi; "
    ...
    if rc != 0:
        ...
        run_logged(
            ...
            f'kill "$pid" >/dev/null 2>&1 || true; fi; rm -f {q(remote_server_pid)}',
    ...
    run_logged(cfg, cfg.remote_a, f"if test -s {q(remote_server_pid)}; then pid=$(cat {q(remote_server_pid)}); kill \"$pid\" >/dev/null 2>&1 || true; fi; rm -f {q(remote_server_pid)}", d / f"{cfg.a_name}_server_cleanup.log")
    print(f"\nLogs: {d}")
    return rc
...
def cmd_configure_acs_grub(cfg: Config, args: argparse.Namespace) -> int:
    ...
    if args.apply and rc == 0:
        print(
            ...
            f"  {launcher} 07\n"
            f"  {launcher} 11\n"
            f"  {launcher} 08 --rail 0 --gdr\n"
            f"  {launcher} 08 --rail 1 --gdr\n\n"
            ...
            f"Use {launcher} 05 --persist instead only when the pair must retain rail IP/MTU "
            "settings across later reboots or repeated performance runs."
        )
    elif args.apply:
        print(
            ...
            f"{launcher} 11 --apply passes on both systems.",
            file=sys.stderr,
        )
    elif rc == 0:
        print("\nPASS: both DUTs satisfy the ACS/Data Direct prerequisite for GDR.")
    else:
        print(
            f"\nSTOP: do not run {launcher} 08 --rail <0-or-1> --gdr until {launcher} 11 "
            "passes on both DUTs.",
            file=sys.stderr,
        )
    print(f"\nLogs: {d}")

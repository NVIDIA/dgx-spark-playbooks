...
ib_write_bw_help="$("${ib_write_bw_path}" --help 2>&1 || true)"
# Bash's <<< here-string feeds the captured help text to grep without running
# ib_write_bw again; some builds print valid help but return a nonzero status.
if grep -Fq -- '--use_cuda_dmabuf' <<<"${ib_write_bw_help}" && \
   grep -Fq -- '--use_data_direct' <<<"${ib_write_bw_help}"; then
  dmabuf_data_direct_ready=1
  echo "PASS: ib_write_bw supports the CUDA DMA-BUF and Data Direct flags required by the GDR bandwidth-test action"
else
  warn "${ib_write_bw_path} does not advertise the CUDA DMA-BUF/Data Direct flags required by the control-host GDR bandwidth-test action"
fi
...
if [[ "${warn_count}" == "0" ]]; then
  if [[ "${dmabuf_data_direct_ready}" == "1" || "${peermem_ready}" == "1" ]]; then
    echo "PASS: RoCEv2 runtime basics configured; run the control-host ACS/Data Direct prerequisite check before any GDR bandwidth test"
  else
    echo "PASS: RoCEv2 basics configured; GPUDirect path not proven by this step"
  fi
else
  echo "PASS: RoCEv2 basics configured with ${warn_count} warning(s); continue with the control-host basic-validation action"
fi

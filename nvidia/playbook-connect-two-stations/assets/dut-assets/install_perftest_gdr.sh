...
#
# This script runs on one DUT. It is intentionally optional: use it before GDR
# only when the control-host RoCE/GPUDirect configuration action reports that
# the installed ib_write_bw lacks --use_cuda_dmabuf or --use_data_direct.
...
perftest_help() {
  local binary="${1:-}"
  if [[ -z "${binary}" ]]; then
    binary="$(ib_write_bw_bin)" || return 1
  fi
  [[ -x "${binary}" ]] || return 1
...
perftest_has_gdr_flags() {
  local binary="${1:-}"
  local help_text
  help_text="$(perftest_help "${binary}")" || return 1
...
print_perftest_identity() {
  local binary="${1:-}"
  if [[ -z "${binary}" ]]; then
    binary="$(ib_write_bw_bin || true)"
  fi
  if [[ -n "${binary}" && -x "${binary}" ]]; then
    info "ib_write_bw path: ${binary}"
    "${binary}" --version 2>&1 | sed 's/^/INFO: ib_write_bw version: /' || true
  else
...
try_os_package_upgrade() {
  ...
  if [[ ! -x "${OS_IB_WRITE_BW}" ]]; then
    warn "OS perftest package did not install ${OS_IB_WRITE_BW}"
    return 1
  fi
  perftest_has_gdr_flags "${OS_IB_WRITE_BW}"
}
...
if [[ "${SOURCE_ONLY}" == "0" ]]; then
  if try_os_package_upgrade; then
    print_perftest_identity "${OS_IB_WRITE_BW}"

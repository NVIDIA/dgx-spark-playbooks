...
for rail in 0 1; do
  ...
  if command -v nmcli >/dev/null 2>&1 && \
     nmcli -g connection.uuid connection show "${con_name}" >/dev/null 2>&1; then
    note "Deleting temporary NetworkManager profile ${con_name}"
  ...
  if [[ "${DOWN_INTERFACES}" == "1" ]]; then
    if networkmanager_manages_interface "${netif}"; then
      nmcli device disconnect "${netif}" >/dev/null
    else

...
OS_IB_WRITE_BW="${OS_IB_WRITE_BW:-/usr/bin/ib_write_bw}"
...
ib_write_bw_supports_gdr_flags() {
  local binary="$1"
  local help_text

  [[ -x "${binary}" ]] || return 1
  help_text="$("${binary}" --help 2>&1 || true)"
  grep -Fq -- '--use_cuda_dmabuf' <<<"${help_text}" && \
    grep -Fq -- '--use_data_direct' <<<"${help_text}"
}
...
ib_write_bw_bin() {
  ...
  if [[ -n "${PERFTEST_PREFIX}" ]]; then
    ...
    # Prefer a source-built binary only when it provides the required GDR
    # flags. Otherwise use the OS package binary instead of a stale local copy.
    if ib_write_bw_supports_gdr_flags "${candidate}"; then
      echo "${candidate}"
      return 0
    fi
    if [[ -x "${OS_IB_WRITE_BW}" ]]; then
      echo "${OS_IB_WRITE_BW}"
      return 0
    fi
    if [[ -x "${candidate}" ]]; then
...
runtime_connection_name() {
  ...
  local rail="$1"
  case "${rail}" in
    0|1) echo "cx8-playbook-${ROLE}-rail${rail}-runtime" ;;
    *) die "Invalid rail: ${rail}" ;;
  esac
}
...
networkmanager_manages_interface() {
  ...
  managed="$(LC_ALL=C nmcli -g GENERAL.NM-MANAGED device show "${netif}" 2>/dev/null || true)"
  [[ "${managed}" == "yes" ]]
}
...
ensure_rail_ip_present() {
  ...
  ip -o -4 addr show dev "${netif}" | grep -F " ${cidr} " >/dev/null || \
    die "${netif} missing ${cidr}; rerun the control-host rail IP/MTU configuration action, selecting persistent mode when the rail IPs must survive reboot/network-manager refresh"
}

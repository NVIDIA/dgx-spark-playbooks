...
for rail in 0 1; do
  ...
  ip addr show dev "${netif}" | grep -F "${cidr}" >/dev/null || \
    die "${netif} missing ${cidr}; rerun the control-host rail IP/MTU configuration action in temporary mode"
  ...
  if [[ -s "${ethtool_err}" ]]; then
    # Some mlx5/netlink combinations return the required link fields while
    # also emitting this harmless unprivileged notice. Preserve every other
    # stderr line, and let the link/speed assertions below remain authoritative.
    grep -vFx 'netlink error: Operation not permitted' "${ethtool_err}" \
      | sed 's/^/WARN: ethtool stderr: /' || true
  fi
  grep -E 'Speed:|Link detected:|Port:|Transceiver:' "${ethtool_log}" || true
  grep -F 'Link detected: yes' "${ethtool_log}" >/dev/null || \
    die "${netif} link is not detected; rerun the control-host cable/link presence action and inspect rail${rail} cable/port"
...
if lsmod | grep -E '^nvidia_peermem' >/dev/null; then
  ...
  echo "INFO: The control-host GDR bandwidth-test action can use CUDA DMA-BUF/Data Direct if ib_write_bw supports those flags"
fi

...
validate_one() {
  ...
  if [[ "${rc}" != "0" ]]; then
    ...
    if grep -Fq 'rerun the control-host rail IP/MTU configuration action' "${log}"; then
      ...
      printf '%s\n' "${color_yellow}ACTION: ${label} has a CX8 rail link-down condition; rerun ./04_check_cable_presence.sh and inspect the reported rail cable/port before rerunning ./07_validate_setup.sh${color_reset}"
    elif grep -Eq 'link is detected but speed is not 400G|Speed: 200000Mb/s|Speed: 200Gb/s' "${log}"; then
      printf '%s\n' "${color_yellow}ACTION: ${label} has a CX8 rail link running below the expected 400G rate; rerun ./04_check_cable_presence.sh, check the reported rail/cable speed and module temperature, and reseat/swap the cable or port before rerunning ./07_validate_setup.sh${color_reset}"
    fi
    return "${rc}"
  fi
}

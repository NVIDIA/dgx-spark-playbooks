...
probe_one() {
  ...
  ssh_plain "${host}" '
    ...
    if command -v systemctl >/dev/null 2>&1 && systemctl is-active --quiet NetworkManager; then
      echo "networkmanager_status=active"
      if command -v nmcli >/dev/null 2>&1; then
        echo "ok nmcli=$(command -v nmcli)"
      else
        echo "missing nmcli (required while NetworkManager is active)"
      fi
    else
      echo "networkmanager_status=inactive-or-unavailable"
      if command -v nmcli >/dev/null 2>&1; then
        echo "optional_ok nmcli=$(command -v nmcli)"
      else
        echo "optional_missing nmcli"
      fi
    fi

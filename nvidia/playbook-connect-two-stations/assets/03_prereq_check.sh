...
summarize_one() {
  ...
  if grep -q '^nvidia_peermem[[:space:]]' "${log}"; then
    ...
    peermem="not loaded (optional; CUDA DMA-BUF/Data Direct may be used by the ./08_run_perftest_pair.sh GDR test)"
  else
    peermem="unknown"
  fi
  ...
  if [[ "${rc}" != "0" ]]; then
    if grep -q "bash\\\\r" "${log}"; then
      status="FAIL: remote helper has CRLF line endings; rerun ./02_push_assets.sh to repush normalized assets"
    elif grep -q 'No such file or directory' "${log}"; then
      status="FAIL: remote helper missing; rerun ./02_push_assets.sh to copy assets"
    else
